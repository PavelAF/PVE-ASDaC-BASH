...
function check_config() {
    ...
    [[ "$1" == 'check-only' ]] && {
        ...
        [[ "$( echo -n 'тест' | wc -m )" != 4 ]] && { LC_ALL="en_US.UTF-8"; echo_warn $'\n'"Предупреждение: установленная кодировка не поддерживает символы Unicode"; echo_info "Кодировка была изменена на '${c_value}en_US.UTF-8${c_null}'"$'\n'; }
