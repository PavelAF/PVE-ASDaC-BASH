...
Реализация в скрипте: точка входа `autocheck_stand()`, внутри — подфункции выбора конфига (`_ac_select_config_file`), загрузки конфига (`_ac_load_config`), обнаружения пулов (`_ac_discover_pools`), выбора группы и стендов (`_ac_select_group_and_stands`); выполнение проверок и форматирование вывода — `_ac_run_stand`, `_ac_exec_batch`, `_ac_format_results`. Python-хелпер встроен в скрипт (heredoc), извлекается в `/tmp/ac_helper.py` при первом запуске (поддержка `bash <(curl ...) --autocheck <url>`).
